...
base_raw = """
{
    ...
    "script": [
        "#!/bin/bash -e",
        "#SBATCH -t 900:00 -N 1 -n 16 --mem=64000M",
        "#SBATCH -p longq",
...
for seed in [
    ...
    for cache_size in [2*1000*1000, 2000*1000*1000, 2000*1000*1000*1000]:
        for history_size in [1, 6]:
            ...
            cmd = "%d %d /var/scratch/pse740/cache/query_sets/25000_%s.json %d %s" % (history_size, cache_size, seed, BASE_MASK ^ mask, ("/var/scratch/pse740/rl_results/sim-rl-15_%d.json" % i))
            ...
            if i not in skip: res.append(data)
            i += 1
...
with open("projects/sim-rl-15.json", "w") as f:
