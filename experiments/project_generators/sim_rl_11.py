...
for seed in [10]:#, 4156, 2536, 2123, 7058, 1087, 8306]:
    ...
    for cache_size in [2*1000*1000]:
        # for history_size in [1, 6]:
        for history_size in [1]:
            mask = 0
            data = json.loads(base_raw)
            # /var/scratch/pse740/cache/query_sets/25000_
            cmd = "%d %d /var/scratch/pse740/cache/query_sets/25000_%s.json %d %s" % (history_size, cache_size, seed, BASE_MASK ^ mask, ("/var/scratch/pse740/rl_results/sim-rl-11_%d.json" % i))
...
with open("projects/sim-rl-11.json", "w") as f:
