...
base_raw = """
{
    "name": "acquire_measurements_full-fast-%d",
...
for i in range(0, 9128): # full
    ...
    data["script"][5] = "$BUILD_CACHE_PATH/trident/trident benchmark -i $DATABASE_PATH/dbpedia-times-details --query_file $BUILD_CACHE_PATH/trident/experiments/queries_full_small_chunks_max_2min_v6/query_chunk_%d.sparql --results_file $PROJECT_PATH/res.json.lines --histogram_mode generate --histogram_file $PROJECT_PATH/temp.json --repetitions 1" % i
    res.append(data)

with open("projects/acquire_measurements_full-fast.json", "w") as f:
