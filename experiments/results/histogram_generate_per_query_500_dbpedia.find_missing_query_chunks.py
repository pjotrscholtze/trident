...
i = 0
reruns = []
for p in glob("results/histogram-generate-per-query-500-dbpedia/*"):
    if p == "results/histogram-generate-per-query-500-dbpedia/graphs": continue
    dst = "/storage/wdps/trident/experiments/queries/queries_cleaned/query_chunk_%s.sparql" % p.split("histogram-generate-per-query-")[2].split("-500-")[0]
    shutil.copy("/storage/wdps/trident/experiments/queries/queries/query_chunk_%s.sparql" % p.split("histogram-generate-per-query-")[2].split("-500-")[0], 
        dst)
