...
i = 0
reruns = []
for p in glob("results/histogram-generate-per-query-500-dbpedia-cleaned-queries-first-round/*"):
    if p == "results/histogram-generate-per-query-500-dbpedia-cleaned-queries-first-round/graphs": continue
    dst = "/storage/wdps/trident/experiments/queries/queries_cleaned_second_round/query_chunk_%s.sparql" % p.split("histogram-generate-per-query-")[2].split("-500-")[0]
